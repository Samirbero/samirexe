...
char SamirCppFile[16384][256];
#define IncV(var,x) (explain ? (me ? ((var -= (x)) && fprintf(fexplain,"(%d, %d): %s [Black]: %s",Opening(-(x)),Endgame(-(x)),__FUNCTION__,SamirCppFile[Min(__LINE__-1,cpp_length)])) : ((var += (x)) && fprintf(fexplain,"(%d, %d): %s [White]: %s",Opening(x),Endgame(x),__FUNCTION__,SamirCppFile[Min(__LINE__-1,cpp_length)]))) : (me ? (var -= (x)) : (var += (x))))
#else
...
FILE * frec;
#endif
#define SamirCpp "C:/Users/Administrator/Documents/Visual Studio 2010/Projects/Samir/Samir/Samir.cpp"
...
void init_variables()
{
	...
	if (Local) f = fopen(SamirCpp, "r");
	else if (Server) f = fopen("./Server/Samir.cpp", "r");
	else f = fopen("./Client/Samir.cpp", "r");
	while (!feof(f))
	{
...
void init_hash()
{
	...
	sprintf(name, "Samir_HASH_%d", WinParId);
	int initialized = 0;
	if (parent && HASH != NULL)
	{
		initialized = 1;
...
void init_shared()
{
	...
	sprintf(name, "Samir_SHARED_%d", WinParId);
...
#ifdef EXPLAIN_EVAL
	memset(SamirCppFile, 0, 16384 * 256);
	FILE * fcpp;
	fcpp = fopen(SamirCpp, "r");
	for (cpp_length = 0; cpp_length < 16384; cpp_length++)
	{
		if (feof(fcpp)) break;
		fgets(mstring, 65536, fcpp);
		memcpy(SamirCppFile[cpp_length], mstring, 256);
		if (!strchr(SamirCppFile[cpp_length], '\n')) SamirCppFile[cpp_length][255] = '\n';
		char * p;
		for (p = SamirCppFile[cpp_length]; (*p) == '\t'; p++);
		strcpy(mstring, p);
		strcpy(SamirCppFile[cpp_length], mstring);
	}
	cpp_length--;
	fclose(fcpp);
#endif
...
void uci()
{
	...
	{
#ifndef W32_BUILD
		fprintf(stdout, "id name Samir 3 x64\n");
#else
		fprintf(stdout, "id name Samir 3\n");
#endif
...
#ifndef W32_BUILD
	fprintf(stdout, "Samir  x64\n");
#else
	fprintf(stdout, "Samir \n");
#endif
